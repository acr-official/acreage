...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AcrMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("AcrMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("AcrMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("acr-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running AcrMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
